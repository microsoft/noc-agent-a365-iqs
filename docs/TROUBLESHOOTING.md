...
| Health reports enabled but not leader | Another App Service instance owns the blob lease, storage RBAC has not propagated, or a restart tried to recreate the already-leased leader blob and received `LeaseIdMissing` | One leader is expected. Lease acquisition now checks whether `monitor/leader.lock` exists before creating it, so restarts do not write to an actively leased blob. Check another instance first; otherwise verify Storage Blob Data Contributor access. |
