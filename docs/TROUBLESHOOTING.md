...
| An incident retries without a Teams post | Delegated identity/consent is absent, or one of five specialist families was partial/unavailable | Check Application Insights `exceptions` for `Automatic investigation requires the subscribed user's delegated identity and pre-consent`. Have the same subscribed operator invoke Fabric IQ, Work IQ, and RTI IQ interactively and complete consent. A subscription alone is not sufficient; partial automatic results are intentionally not sent. Replay a fresh incident after consent because failed events may be dead-lettered after three attempts. |
