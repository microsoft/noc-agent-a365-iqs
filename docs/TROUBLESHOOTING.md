...
|---|---|---|---|
| Cowork authenticates, lists `noc_investigate`, and invokes it through APIM, but topology answers report `DataAgent_NOCNetworkDataAgent` failed before producing a result | Reproduced independently against the Data Agent's raw Fabric MCP endpoint with an admin Fabric token. `initialize` and `tools/list` returned HTTP 200, while `tools/call` returned `isError: true` with `The Data Agent run failed before producing a result.` The portal now visibly shows all 8 nodes, all 8 edges, and `Data load completed`; the lakehouse tables contain real rows. The saved GraphModel definition also reads back 8 node types/8 edge types with the expected 8 node tables and 8 edge tables. Republished the Data Agent again after the portal load completed; both a simple router query and the shared-conduit query still fail identically. | Authentication, Cowork packaging, APIM, MCP transport, Foundry-agent invocation, graph canvas shape, graph refresh, capacity, and lakehouse row availability are proven healthy. The remaining fault is specifically in the Fabric Data Agent's published Ontology query/execution runtime or its binding to the refreshed graph, not in the visible graph canvas or lakehouse contents. | In the `NOCNetworkDataAgent` Fabric item, force a source refresh/rebind: remove the `NOCNetworkOntology` source from the staging configuration, save, add the same Ontology item again, save, and publish. Then run the two narrow queries in the Data Agent's own test pane and inspect the generated GQL/error. If the portal test still fails, capture the Data Agent run/request ID and raise it as a Fabric Data Agent/Ontology runtime issue; do not rebuild the already-correct graph or lakehouse. Expected result remains: links `LINK-SYD-MEL-FIBRE-01` and `-02` share `CONDUIT-SYD-MEL-INLAND`. |
